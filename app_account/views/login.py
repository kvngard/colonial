...
def process_request(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.urlparams[1] != '':
                return HttpResponseRedirect('/{}/{}/'.format(request.urlparams[0], request.urlparams[1]))
            if request.urlparams[0] != '':
                return HttpResponseRedirect('/{}/'.format(request.urlparams[0]))
    ...
    params['option_link'] = '/app_account/new.signup/'

    params['option2'] = 'Forgot Password?'
    params['option2_link'] = '/password_reset/'

    return templater.render_to_response(request, 'static_login.html', params)
